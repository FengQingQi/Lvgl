...
static void sin_line_anim(lv_obj_t * line, lv_anim_value_t value);
...
static void obj_event_cb(lv_obj_t * obj, lv_event_t e)
{
    static char *buf;
    if(e == LV_EVENT_VALUE_CHANGED)
    {
        buf = lv_msgbox_get_active_btn_text(obj);
        printf("Button: %s\n", buf);

        if(strcmp(buf,"确定")==0)
            lv_label_set_text(info, "Click cancel to close");
        else
...
static void controls_create(lv_obj_t * parent)
{
    ...
    //lv_style_set_border_color(&my_style, LV_STATE_DEFAULT, LV_COLOR_RED);
    lv_style_set_value_color(&my_style, LV_STATE_CHECKED, LV_COLOR_RED);
    lv_style_set_text_color(&my_style, LV_STATE_DEFAULT, LV_COLOR_CYAN);
    lv_style_set_text_color(&my_style, LV_STATE_FOCUSED, LV_COLOR_RED);
    lv_style_set_text_color(&my_style, LV_STATE_PRESSED, LV_COLOR_BLACK);
    lv_style_set_text_font(&my_style, LV_STATE_DEFAULT, &myFont);
    ...
    info = lv_label_create(m, NULL);

    lv_obj_add_style(m, LV_CONT_PART_MAIN, &my_style);

    lv_label_set_text(info, "我的消息box");
    ...
    lv_obj_set_event_cb(m, obj_event_cb);
#endif
    ...
    char* la_txt = "要么庸俗,要么孤独--叔本华\n      QQ:1223419620";
    char *b_Cancel = "取消";
    lv_obj_t * label1 = lv_label_create(lv_scr_act(), NULL);
    lv_obj_add_style(label1, LV_CONT_PART_MAIN, &my_style);
    lv_label_set_text(label1, la_txt);
    lv_obj_set_pos(label1, 120, 2);
    ...
    //lv_obj_add_style(label, LV_CONT_PART_MAIN, &style1);
    ...
    lv_obj_add_style(label, LV_CONT_PART_MAIN, &my_style);
    ...
    lv_obj_set_style_local_transition_time(slider, LV_SLIDER_PART_KNOB, LV_STATE_DEFAULT, 1000);
...
lv_coord_t sin_data[]=
{
    0x7F,0x98,0xB0,0xC6,0xD9,0xE9,0xF5,0xFC,0xFE,0xFC,0xF5,0xE9,0xD9,0xC6,0xB0,0x98
   ,0x7F,0x66,0x4E,0x38,0x25,0x15,0x09,0x02,0x00,0x02,0x09,0x15,0x25,0x38,0x4E,0x66
};
static lv_point_t line_points[sizeof(sin_data)/sizeof(sin_data[0])];
...
//    lv_chart_set_div_line_count(chart, 3, 0);
    lv_chart_set_point_count(chart, sizeof(sin_data)/sizeof(sin_data[0]));
    ...
    lv_obj_set_style_local_line_dash_gap(chart,LV_CHART_PART_SERIES, LV_STATE_DEFAULT, 10);
    lv_obj_set_style_local_line_dash_width(chart,LV_CHART_PART_SERIES, LV_STATE_DEFAULT, 4);
    lv_obj_set_style_local_line_opa(chart,LV_CHART_PART_SERIES, LV_STATE_DEFAULT, 50);
    lv_obj_set_style_local_line_blend_mode(chart,LV_CHART_PART_SERIES, LV_STATE_DEFAULT, 0);
    lv_obj_set_style_local_line_rounded(chart,LV_CHART_PART_SERIES, LV_STATE_DEFAULT, 0);
    lv_obj_set_style_local_line_width(chart,LV_CHART_PART_SERIES, LV_STATE_DEFAULT, 1);
    lv_obj_set_style_local_radius(chart,LV_CHART_PART_SERIES, LV_STATE_DEFAULT, 1);
//    lv_chart_set_series_width(chart, 0);            /*Line width and point radious*/
    ...
    lv_obj_set_style_local_outline_pad(chart, LV_CHART_PART_SERIES, LV_STATE_DEFAULT, 0);
...
//    for(int i=0; i<sizeof(sin_data)/sizeof(sin_data[0]); i++)
//    {
//        line_points[i].x = 2*i+20;
//        line_points[i].y = sin_data[i];
//    }
//    /*Create a line and apply the new style*/

//    line1 = lv_line_create(chart, NULL);
//    lv_line_set_points(line1, line_points, sizeof(sin_data)/sizeof(sin_data[0]));     /*Set the points*/
//    lv_obj_add_style(line1, LV_LINE_PART_MAIN, &style_line);     /*Set the points*/
//    lv_obj_align(line1, NULL, LV_ALIGN_CENTER, 0, 0);

//    lv_anim_t a_line;
//    lv_anim_init(&a_line);
//    lv_anim_set_var(&a_line, chart);
//    lv_anim_set_exec_cb(&a_line, (lv_anim_exec_xcb_t)sin_line_anim);
//    lv_anim_set_values(&a_line, 0, 100);
//    lv_anim_set_time(&a_line, 4000);
//    lv_anim_set_playback_time(&a_line, 1000);
//    lv_anim_set_repeat_count(&a_line, LV_ANIM_REPEAT_INFINITE);
//    lv_anim_start(&a_line);


    for(int i=0; i<sizeof(sin_data)/sizeof(sin_data[0]); i++)
        sin_data[i]=sin_data[i]/3;
    lv_chart_set_points(chart, s1, sin_data);
    ...
    lv_obj_t * chart2 = lv_chart_create(parent, NULL);
    ...
    lv_gauge_set_needle_img(gauge, &img_hand, 4, 4);
...
static void sin_line_anim(lv_obj_t * chart, lv_anim_value_t value)
{
    static int indx=0;
    int cnt=sizeof(sin_data)/sizeof(sin_data[0]);
    for(int i=0; i<cnt; i++)
    {
        line_points[i].x = 2*i+20;
        line_points[i].y = sin_data[i];
    ...
    indx = (indx+1)%cnt;
    lv_line_set_points(line1, line_points, cnt);     /*Set the points*/
    line1 = lv_line_create(chart, NULL);
    if(indx&0x01)
